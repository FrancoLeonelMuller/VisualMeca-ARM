...
#Region "Variables"
    ...
    Dim crearSec As Boolean = False
    Dim pasosCreSec As Integer = 0
    Dim AntPosBuffer As String
    Dim ParaGuardar As Boolean = False
...
#Region "Teclas JOG"
    Private Sub JOD_KEYDown(ByVal sender As Object, ByVal e As KeyEventArgs) Handles Screens.KeyDown
        If EstadoModoJOG Or EstadoModoCSEC Then
    ...
    Public Sub ReproducirSecuencia()
        If repSec Then
            If PasoSecAct < File.ReadAllLines(SecAct).Length Then
                If PasoSecAct = 0 And buffer.Contains("*HOME*") Then
                    PasoSecAct += 1
...
#Region "Crear"
    ...
    Private Sub BtnDelaySec_Click(sender As Object, e As EventArgs) Handles btnDelaySec.Click
        pasosCreSec += 1
        Using writer As New StreamWriter("temp.sec", True)
            writer.WriteLine("delay(" + cmbDelaySec.Value.ToString + ")")
        End Using
    End Sub
    ...
    Public Sub CrearSecuencia()
        ...
        If crearSec And pasosCreSec = 0 Then
            Dim tempSEC As New System.IO.StreamWriter("temp.sec")
            tempSEC.WriteLine("*HOME*")
            tempSEC.WriteLine("*S*")
            tempSEC.Close()
            pasosCreSec += 1
        End If

        If pasosCreSec = 1 And buffer.Contains("*HOME*") Then
        ...
        If pasosCreSec > 1 And ParaGuardar And AntPosBuffer <> Posbuffer Then

            Using writer As New StreamWriter("temp.sec", True)
                writer.WriteLine(Posbuffer)
                writer.WriteLine(AntPosBuffer)
            End Using
...
#Region "Funciones Visuales"
    ...
    Public Sub Inicializar()
        ...
        Clear.Enabled = False
        BtnSend.Enabled = False
        cmbStepEje1.SelectedItem = "8"
        cmbStepEje2.SelectedItem = "8"
        cmbStepEje3.SelectedItem = "16"
        cmbStepEje4.SelectedItem = "16"
...
#End Region
    ...
    Private Sub While_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Timer1.Tick
        ...
        Try
            If EstadoModoCSEC Then
                CrearSecuencia()
            End If
        Catch ex As Exception
            MsgBox("Ocurrio un error en crear la secuencia, reiniciando modo")
        End Try
